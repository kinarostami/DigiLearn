...
public class BaseRazor : PageModel
{
    public override void OnPageHandlerExecuting(PageHandlerExecutingContext context)
    {
        if (context.HandlerMethod.MethodInfo.Name == "OnPost")
            ...
            {
                var model = Newtonsoft.Json.JsonConvert.SerializeObject(OperationResult.Error(JoinErrors()));
    ...
    {
        var model = Newtonsoft.Json.JsonConvert.SerializeObject(result);
        HttpContext.Response.Cookies.Append("SystemAlert", model);
    ...
    {
        var model = Newtonsoft.Json.JsonConvert.SerializeObject(OperationResult.Success());
        HttpContext.Response.Cookies.Append("SystemAlert", model);
    }
    protected void SuccessAlert(string message)
    {
        var model = Newtonsoft.Json.JsonConvert.SerializeObject(OperationResult.Success(message));
        HttpContext.Response.Cookies.Append("SystemAlert", model);
    }
    protected void ErrorAlert()
    {
        var model = Newtonsoft.Json.JsonConvert.SerializeObject(OperationResult.Error());
        HttpContext.Response.Cookies.Append("SystemAlert", model);
    }
    protected void ErrorAlert(string message)
    {
        var model = Newtonsoft.Json.JsonConvert.SerializeObject(OperationResult.Error(message));
        HttpContext.Response.Cookies.Append("SystemAlert", model);
    }
        ...
        bool isErrorReloadPage = false)
    {
        try
        {
            if (!ModelState.IsValid)
            {
                ...
                var jsonResult = Newtonsoft.Json.JsonConvert.SerializeObject(modelError);
                return Content(jsonResult);
            }
