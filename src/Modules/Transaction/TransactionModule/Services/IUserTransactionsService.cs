...
{
    #region Command
    Task<Guid> CreateTransaction(CreateTransactions command);
...
public class UserTransactionsService : IUserTransactionsService
{
    ...
    public UserTransactionsService(TransactionContext context)
    {
        _context = context;
        //_userAcl = userAcl;
    }
    public async Task<Guid> CreateTransaction(CreateTransactions command)
    {
        var transaction = new UserTransaction(command.UserId, command.PaymentAmount, command.TransactionFor, command.LinkId,
    ...
    {
        return await _context.UserTransactions.FirstOrDefaultAsync(x => x.Id == id);
    }
